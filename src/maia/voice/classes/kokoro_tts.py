...
class KokoroTTS(TTSEngine):
    ...
    def generate_audio(self, text: str, output_file: str, voice: int):
        ...
        generator = self.pipeline(
            text,
            voice="ff_siwis",  # <= change voice here
            speed=1,
            split_pattern=r"\n+",
        )
        ...
        sf.write(output_file, full_audio.cpu().numpy(), 24000)
    ...
    async def generate_audio_stream(
        ...
        generator = self.pipeline(
            text,
            voice="ff_siwis",  # <= change voice here (only one FR voice)
            speed=1,
            split_pattern=r"\n+",
        )
    ...
    def _write_wav(file: str, audio: torch.Tensor):
        sf.write(file, audio.cpu().numpy(), 24000)
