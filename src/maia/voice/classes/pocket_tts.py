...
class PocketTTS(TTSEngine):
    ...
    def generate_audio(self, text: str, output_file: str, voice: int):
        ...
        audio = self.tts_model.generate_audio(
            self.voice_states[voice], f"{text}", frames_after_eos=2
        )

        sf.write(output_file, audio.cpu().numpy(), self.tts_model.sample_rate)
    ...
    async def generate_audio_stream(
        ...
        while i < len(chunks):
            ...
            res = await asyncio.to_thread(
                self._write_wav, chunk_file, audio, self.tts_model.sample_rate
            )
    ...
    @staticmethod
    def _write_wav(file: str, audio: torch.Tensor, samplerate: int) -> bool:
        sf.write(file, audio.cpu().numpy(), samplerate)
